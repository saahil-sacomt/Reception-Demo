...
const SpecialWorkOrderGeneration = ({ isCollapsed }) => {
  ...
  // Generate a new Sales Order ID
  const generateNewWorkOrderId = useCallback(async () => {
    try {
      console.log("Attempting to generate new Sales Order ID...");

      // Define default starting Sales Order IDs per branch
      ...
      // Fetch the last Sales Order ID for the current branch
      ...
      if (error) {
        ...
        // alert("Error generating Sales Order ID. Please try again.");
        return;
      }

      console.log("Last Sales Orders Retrieved for branch:", lastWorkOrders);

      let newWorkOrderId = branchDefaultIds[branch] || 1001; // Default if no Sales Orders exist for branch

      if (lastWorkOrders && lastWorkOrders.length > 0) {
        const lastWorkOrderId = parseInt(lastWorkOrders[0].work_order_id, 10);
        console.log("Last Sales Order ID for branch:", lastWorkOrderId);
      ...
      console.log("Generated Sales Order ID:", newWorkOrderId);
    } catch (error) {
      console.error("Error generating Sales Order ID:", error);
      alert("An unexpected error occurred while generating Sales Order ID.");
    }
  }, [dispatch, branch]);
  ...
  const fetchProductSuggestions = useCallback(async (query, type, index) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`product_id, product_name, mrp, hsn_code,sgst,cgst`)
        .ilike(column, `%${query}%`)
        .limit(10);
              ...
              productEntries: productEntries.map((entry, i) =>
                i === index
                  ? {
                    ...
                    sgst: productDetails.sgst || 6,
                    cgst: productDetails.cgst || 6
                  }
                  : entry
              ),
            },
  ...
  const calculateTotals = useCallback((entries, discountAmt) => {
    ...
    validDiscountAmount = Math.min(parseFloat(discountAmt) || 0, subtotal);

    // Process each product individually for detailed calculations
    let productsCalculation = entries.map(product => {
      const price = parseFloat(product.price) || 0;
      const quantity = parseInt(product.quantity) || 0;
    ...
    return {
      ...
      productsCalculation // Include this for detailed product calculations if needed
    };
  }, []);
  ...
  const fetchModificationRequest = async (orderId, orderType) => {
    try {
      const { data, error } = await supabase
  ...
  const {
    ...
    discountedTotal = 0,
    discountedSubtotalwithoutGST = 0,
  } = useMemo(
  ...
  const balanceDue = Math.max(totalAmountWithGST - advance, 0);
  ...
  const saveWorkOrder = useCallback(async () => {
    ...
    const validations = [
      ...
      // Payment method validation removed
    ];
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (error) {
          if (error.status === 409) {
            alert("Sales Order ID already exists. Generating a new ID...");
        ...
        if (!workOrderId) {
          alert("Sales Order ID is not generated yet. Please wait.");
          dispatch({
            type: "SET_WORK_ORDER_FORM",
            payload: { isSaving: false },
          });
        ...
        const { error } = await supabase.from("specialworkorders").insert(payload);

        if (error) {
          if (error.status === 409) {
            alert("Sales Order ID already exists. Please try saving again.");
            // Optionally, regenerate Sales Order ID
  ...
  // Fetch existing Sales Order details for editing
  const fetchWorkOrderDetails = useCallback(async () => {
    try {
      ...
      console.error("Unexpected error fetching Sales Order details:", err);
  ...
  return (
    ...
    >
      ...
      >
        {/* Step 1: Sales Order ID and Product Entries */}
        {step === 1 && (
          ...
          <div className="w-fit bg-gray-50 p-6 rounded-md shadow-inner space-y-4">
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">
                {isEditing ? "Sales Order ID" : "Generated Sales Order ID"}
              </label>
              <input
                type="text"
                value={workOrderId}
        ...
        {step === 5 && (
          <>
            ...
            <div className=" bg-white rounded-lg text-gray-800">
              <div className="printable-area print:mt-20 print:block print:absolute print:inset-0 print:w-full bg-white p-4 print:m-0 print:p-0 w-full">
                ...
                <div className="print:hidden flex flex-col md:flex-row items-center justify-between my-6 space-x-4">
                  ...
                  <div className="w-full print:w-1/2 mb-4 md:mb-0">
                    <label
                      htmlFor="discount"
                ...
                <div className="flex-col justify-start mx-auto items-start text-left text-md">
                  ...
                  <p className="mt-2 text-xs">
                    ...
                    <ol className="list-decimal list-inside">
                      <li>Sales Order valid only for two months.</li>
                      <li>
            ...
            {/* Save Sales Order and Print Buttons */}
...
export default SpecialWorkOrderGeneration;
