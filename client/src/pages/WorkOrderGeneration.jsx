...
const WorkOrderGeneration = ({ isCollapsed }) => {
  ...
  //     if (!branch || !subRole) {
  //       console.error("Branch or subRole is undefined. Cannot generate Work Order ID.");
  //       alert("Branch or department information is missing. Please try again.");
  //       return;
  //     }

  //     // Use a single database call to atomically increment and retrieve the next ID
  //     const { data, error } = await supabase.rpc('get_next_work_order_id', {
  //       branch_code: branch,
  //       role_code: subRole
  //     });
  ...
  //     console.log("Database returned next ID:", data);

  //     // Determine the format prefix based on role
  //     let prefix;
  //     if (role && role.includes("opd")) {
  //       // Extract OP Number from subRole
  //       const opNumber = subRole ? subRole.split(" ")[1] || "01" : "01";
  //       prefix = `OPW-${opNumber}`;
  //     } else {
  //       // Extract OP Number for counselling
  //       const opNumber = subRole ? subRole.split(" ")[1] || "01" : "01";
  //       prefix = `CR-${opNumber}`;
  //     }

  //     // Format the new ID using the sequence number
  //     const formattedId = `${prefix}-${String(data).padStart(3, "0")}`;
  //     console.log("Final Generated Work Order ID:", formattedId);

  //     dispatch({
  //       type: "SET_WORK_ORDER_FORM",
  //       payload: { workOrderId: formattedId.toString() },
  //     });
  //   } catch (error) {
  //     console.error("Unexpected error generating Work Order ID:", error);
  //     alert("An unexpected error occurred while generating Work Order ID.");
  //   }
